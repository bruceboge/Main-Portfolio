...
function initializeFilters() {
  ...
  // Mobile Filter
  const select = document.querySelector("[data-select]"); // Get the select button reference
  selectItems.forEach((item) => {
    item.addEventListener("click", function () {
      const selectedValue = this.getAttribute("data-category");
      const selectValue = document.querySelector("[data-select-value]");
      ...
      selectValue.innerText = this.innerText;
      select.classList.remove("active"); // Remove active from the BUTTON to hide list
      filterFunc(selectedValue);
    });
  });
}
